...
bool CFont::SetWriteBuff( SCharRenderObject* renderBuff, int buffSize )
{
	this->renderBuff = renderBuff;
	this->renderBuffOffset = 0;
	this->renderBuffSize = buffSize;
	return true;
}

int CFont::GetWriteCount()
{
	return renderBuffOffset;
}

void CFont::InternalWrite(float x, float y, float z, const char *text, int count, float spacing)
{
	if (renderBuff == NULL) return;
	...
	for( int n = 0; n < count; )
	{
		if (renderBuffSize <= renderBuffOffset) return;
		SCharRenderObject* renderObject = renderBuff + renderBuffOffset;
		++ renderBuffOffset;
