...
public:
	...
	bool SetWriteBuff(SCharRenderObject* renderBuff, int buffSize);
	int GetWriteCount();

	int GetPageCount();
	std::string GetPagePath(int pageId);
...
protected:
	...
	SCharRenderObject* renderBuff;
	int renderBuffSize;
	int renderBuffOffset;
};
