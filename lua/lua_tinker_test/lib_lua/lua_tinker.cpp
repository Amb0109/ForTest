...
static void call_stack(lua_State* L, int n)
{
    ...
    if(lua_getstack(L, n, &ar) == 1)
	{
		...
		if(ar.name)
			lua_tinker::print_error(L, "%s%s() : %s (%d)", indent, ar.name, ar.source, ar.currentline);
		else
			lua_tinker::print_error(L, "%sunknown : %s (%d)", indent, ar.source, ar.currentline);
...
void lua_tinker::string_lua_value::to_lua( lua_State *L )
{
...
template<>	
lua_tinker::string_lua_value lua_tinker::pop(lua_State *L)
{
	string_lua_value ret;
	ret.pstr = lua_tolstring(L, -1, &ret.len);
	lua_pop(L, 1);
	return ret;
}
