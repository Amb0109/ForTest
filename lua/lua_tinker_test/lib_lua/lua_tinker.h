...
namespace lua_tinker
{
	...
	struct lua_value
	{
		...
		lua_value* arg() {return this;}
	};

	struct string_lua_value : lua_value // add by huangwei, 2013-8-29
	{
		virtual void to_lua(lua_State *L);
	...
	template<>	string_lua_value	pop(lua_State *L); // add by huangwei, 2013-8-29
	...
	template<typename T>
	int meta_tostring(lua_State *L) // add by huangwei, 2013-8-29
	{
		char buff[32];
		val2user<T>* obj = (val2user<T>*)lua_touserdata(L, 1);
		sprintf_s(buff, "%p", obj->m_p);
		lua_pushfstring(L, "%s (%s)", class_name<T>::name(), buff);
		return 1;
	}
	...
	{ 
		...
		// add by huangwei 2013-8-29
		lua_pushstring(L, "__tostring");
		lua_pushcclosure(L, meta_tostring<T>, 0);
		lua_rawset(L, -3);
